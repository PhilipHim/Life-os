...
import { getRecurringTemplates, clearRecurringDataForTemplate } from '@/features/tasks/lib/recurring'
import { removePlannerDefaults } from '@/features/planner/lib/planner-defaults'
import { removeDailyPlanItemsByWorkItemId } from '@/database/daily-plan'
import {
...
export function WorkItemProvider({ children }: { children: ReactNode }) {
  ...
  const addWorkItem = (title: string, type: 'single' | 'group', description?: string, childrenIds?: string[]) => {
    ...
    markFirstMissionObjective('task')
    checkFirstMissionCompletion()
  }
